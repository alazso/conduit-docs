...
<Callout type="info" title="Want the latest beta?">
Pre-release builds are published as snapshots in a separate repository. To use the bleeding-edge `CONDUIT_SNAPSHOT_VERSION` build, point at the snapshots repo instead:
...
dependencies {
    compileOnly("so.alaz.conduit:conduit-api:CONDUIT_SNAPSHOT_VERSION")
}
```
</Callout>
